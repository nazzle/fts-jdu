...
use frontend\models\PositionsSearch;
...
class PositionsController extends Controller
{
    ...
    /**
     * 
     * This action is to find all the the user_id associating
     * with the selected positions id's.
     */
    public function actionUserid($id)
    {
    
        $countPositionsids = Positions::find()
                ->where(['id'=>$id])
                ->count();
        
        $positionsids = Positions::find()
                ->where(['id'=> $id])
                ->all();
        
        if ($countPositionsids > 0)
        {
            foreach($positionsids as $positionsid) {
                echo "<option value='".$positionsid->user_id."'>".$positionsid->user_id."</option>";
            } 
        }else {
                echo "<option>-</option>";
